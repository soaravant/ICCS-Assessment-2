...
def train_one_epoch(
    ...
    # Select the appropriate autocast context based on the active device
    def _get_autocast_ctx():
        if torch.cuda.is_available():
            return torch.cuda.amp.autocast()
        # On some PyTorch versions, MPS autocast is unsupported. Use full precision on MPS.
        if getattr(torch.backends, "mps", None) is not None and torch.backends.mps.is_available():
            return contextlib.nullcontext()
        # Fallback: no autocast
        return contextlib.nullcontext()

    autocast_ctx_factory = _get_autocast_ctx
    ...
    for step, (images, targets) in progress_bar:
        ...
        with autocast_ctx_factory():
            ...
            losses = total_loss / grad_accum_steps
        ...
        # Clear gradients and detach tensors to prevent memory accumulation
        with torch.no_grad():
            detached_losses = {k: v.detach() for k, v in loss_dict.items()}
            detached_losses["loss"] = total_loss.detach()
            metrics.update(detached_losses)

            if (step % 10) == 0:
                progress_bar.set_postfix({"loss": detached_losses["loss"].item()})
    ...
    remainder = steps_processed % grad_accum_steps
    if remainder != 0:
        if scaler is not None:
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_(model.parameters(), config.GRAD_CLIP_NORM)
